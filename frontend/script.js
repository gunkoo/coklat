  ...
  showConfirmModal({
    ...
    onConfirm: async function () {
      ...
      try {
        ...
        localStorage.setItem('userDatabase', JSON.stringify(users));
        localStorage.setItem('userDatabase_cache', JSON.stringify(users));
        showNotification({ type: 'success', message: 'Masa aktif ' + username + ' diperpanjang 30 HARI! (lokal)' });
        const container = document.getElementById('kelolaUserContent');
        if (container) renderUserManagementContent(container);
      }
    }
  ...
  showConfirmModal({
    ...
    onConfirm: async function () {
      ...
      try {
        ...
        if (r.ok) {
          await syncUsersFromServer();
          showNotification({ type: 'success', message: 'User ' + username + ' berhasil dihapus dari server!' });
        } else if (r.status === 404) {
          showNotification({ type: 'warning', message: 'User ' + username + ' tidak ditemukan di server.' });
        } else {
          showNotification({ type: 'error', message: 'Gagal: ' + (r.data?.error || r.status) });
  ...
  try {
    ...
    if (session.valid === false) {
      ...
      const reasons = {
        ...
        EXPIRED: '⏱️ Masa aktif akun telah habis.',
      };
